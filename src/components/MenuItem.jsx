...
const MenuItem = ({ food, initialRating }) => {
  const [hoverRating, setHoverRating] = useState(null);

  const { theme, cart, addToCart, subtract } = useTheme();
  const count = cart[food.id] || 0;

  // Utilisez initialRating pour définir la note initiale
  ...
  return (
    <div key={food.id} className={`${bgClass} ${textClass} rounded-lg shadow-lg transition duration-300 flex flex-col`}>
      <div className="relative">
        <img src={`src/assets/images/food_list/${food.image}`} alt={food.name} className="w-full h-[220px] object-cover rounded-t-lg" />
      </div>
